create view if not exists prd.missing_cusips_view_ARM_2018_03 as
...
drop view prd.fhlmonly_view_ARM_2018_3;

create view if not exists prd.fhlmonly_view_ARM_2018_3 as
select substr(loan_identifier, length(loan_identifier)-3) as loan_num,cusip,
...
from prd.fhlmonly where year=2018 and month=3 and amortization_type='ARM';


drop view prd.fhlmc_loan_sqoop_ARM_view;
create view if not exists prd.fhlmc_loan_sqoop_ARM_view as
select f.loan_seq_num as loan_num,p.cusip, 
...
left outer join prd.pool_sqoop p on (f.product_id = p.product_id)
where  f.prod_type_ind = 'A' and f.current_upb>0 
and f.last_chg_date >=unix_timestamp('2018-03-06','yyyy-MM-dd')*1000
and p.cusip not in (select cusip from missing_cusips_view_arm_2018_03)

select from_unixtime(first_pi_date div 1000, 'MMyyyy'),first_pi_date
from prd.fhlmc_loan_sqoop_ARM_view where cusip='3128NCPT5' limit 10

select * from prd.fhlmc_loan_sqoop_ARM_view a 
full outer join prd.fhlmonly_view_ARM_2018_3 b 
on a.cusip=b.cusip and a.loan_num=cast(b.loan_num as int)
where a.loan_num is null or b.loan_num is null
___________________________________________________
create view prd.missing_cusips_view_ARM_2018_03 as
select t1.cusip from
(select distinct p.cusip  from prd.fhlmc_loan_sqoop f 
left outer join prd.pool_sqoop p on (f.product_id = p.product_id)
where  f.prod_type_ind = 'A' and f.current_upb>0 and
f.last_chg_date >=unix_timestamp('2018-03-06','yyyy-MM-dd')*1000
) t1 
left outer join(
select distinct cusip from prd.fhlmonly 
where year=2018 and month=3 and amortization_type='ARM'
...
select * from prd.missing_cusips_view_ARM_2018_03;


 
drop view prd.fhlmonly_view_ARM_2018_3;
create view prd.fhlmonly_view_ARM_2018_3 as
select substr(loan_identifier, length(loan_identifier)-3) as loan_num,cusip,
issuance_investor_loan_upb as orig_upb, current_investor_loan_upb as current_upb,
amortization_type as amort_type, original_interest_rate as orig_note_rate,
issuance_interest_rate as pc_issuance_note_rate,
...
select distinct first_pi_date from prd.fhlmonly_view_ARM_2018_3;
...
f.orig_upb,f.current_upb,concat(f.prod_type_ind,'','RM') as amort_type,
f.orig_note_rate,f.pc_issuance_note_rate, f.orig_loan_term,
f.rem_months_to_maturity,f.loan_age,f.io_flag,
cast(from_unixtime(first_pi_date div 1000, "MMyyyy") as bigint) as first_pi_date,
if(f.io_flag='N',null, f.months_to_amortize) as months_to_amortize,
...
where  f.prod_type_ind = 'A' and f.current_upb>0 
and f.last_chg_date >=unix_timestamp('2018-03-06','yyyy-MM-dd')*1000
and m.last_chg_date >=unix_timestamp('2018-03-06','yyyy-MM-dd')*1000
and p.cusip not in (select cusip from missing_cusips_view_arm_2018_03)

select distinct first_pi_date 
from prd.fhlmc_loan_sqoop_ARM_view limit 5;
