...
public class FHLMCOLDFileParser extends BaseFileParser {
	...
	public CSVWriter getPen(RecordTypeFHLMCOLD recordType) throws IOException {
		if (recordType == RecordTypeFHLMCOLD.LOAN)
			return loanPen;
		else if (recordType == RecordTypeFHLMCOLD.MODIFIED_LOAN)
	...
	public FHLMCOLDFileParser(String inputFileName) {
		super();
		...
		String outputLoanFileName = sb.append(elements[0]).append("_")
				.append(RecordTypeFHLMCOLD.LOAN).append(".").append(elements[1])
				.toString();
		try {
		...
		outputLoanFileName = sb.append(elements[0]).append("_")
				.append(RecordTypeFHLMCOLD.MODIFIED_LOAN).append(".").append(elements[1])
				.toString();
		try {
	...
	public FHLMCOLDFileParser(String inputFileName, String outputFileName) {
		super();
		this.inputFileName = inputFileName;
		...
		try {
			...
			while ((line = reader.readNext()) != null) {
				if (StringUtils.isEmpty(line[0])
						|| RecordTypeFHLMCOLD.from(Integer.valueOf(line[0])) == null)
					continue;
				RecordTypeFHLMCOLD recordType = RecordTypeFHLMCOLD.from(Integer
						.valueOf(line[0]));
